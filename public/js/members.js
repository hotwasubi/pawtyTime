$(document).ready(function() {
  ...
  if(data.length > 0){
    for(let i = 0; i < data.length; i++){
     return $("#appointments").append("<li class='collection-item'>>" + data[i] + "</li>")
    }
  } else {
     return $("#appointments").append("<li class='collection-item'>>You have no upcoming appointments!</li>")
  }
  })
  }
  ...
  function getDogData(){
    $.get("/api/dog").then(results => {
      for(let i = 0; i < results.length; i++){
       $("#petList").append(`<li class='collection-item'>` + results[i] + `<a class='secondary-content' id='editDog'><i class='material-icons green-text'>edit</i></a><a class='secondary-content' id='deleteDog'><i class='material-icons red-text'>delete_forever</i></a></li>`) 
      }
    })
  }
